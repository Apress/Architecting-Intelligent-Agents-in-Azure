...
class CosmosApprovalStore:
    ...
    async def _ensure_container(self) -> Any:
        ...
        try:
            self._database = await execute_dependency_call(
                "cosmos",
                "cosmos.database.create_if_not_exists",
                lambda: self._client.create_database_if_not_exists(id=self._config.database),
            )
        ...
        try:
            ...
            self._container = await execute_dependency_call(
                "cosmos",
                "cosmos.container.create_if_not_exists",
                lambda: self._database.create_container_if_not_exists(
            ...
            async def _get_container() -> Any:
                return self._database.get_container_client(self._config.container)

            self._container = await execute_dependency_call(
                "cosmos",
                "cosmos.container.get",
                _get_container,
            )
        except Exception as exc:
    ...
    async def create_request(self, record: dict[str, Any]) -> dict[str, Any]:
        ...
        try:
            await execute_dependency_call(
                "cosmos",
                "cosmos.approvals.create_item",
                lambda: container.create_item(payload),
            )
            return payload
        except Exception as exc:
    ...
    async def get(self, approval_id: str) -> dict[str, Any] | None:
        ...
        try:
            return await execute_dependency_call(
                "cosmos",
                "cosmos.approvals.read_item",
                lambda: container.read_item(item=approval_id, partition_key=approval_id),
            )
    ...
    async def record_decision(
        ...
        *,
        ...
        try:
            return await execute_dependency_call(
                "cosmos",
                "cosmos.approvals.record_decision",
    ...
    async def mark_executed(self, approval_id: str, executor_run_id: str | None = None) -> bool:
        ...
        try:
            await execute_dependency_call(
                "cosmos",
                "cosmos.approvals.mark_executed",
                lambda: container.replace_item(
                    item=approval_id,
                    body=updated,
                    etag=etag,
