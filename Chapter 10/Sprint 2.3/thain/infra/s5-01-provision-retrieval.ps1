...
if ($SkipAuthUpdate) {
    Write-Host "Skipping Search auth update (-SkipAuthUpdate)."
} else {
    $currentAuthMode = az search service show `
        --name $SearchServiceName `
        --resource-group $ResourceGroupName `
        --query "keys(authOptions)[0]" -o tsv `
        --only-show-errors 2>$null
    $currentFailureMode = az search service show `
        --name $SearchServiceName `
        --resource-group $ResourceGroupName `
        --query "authOptions.aadOrApiKey.aadAuthFailureMode" -o tsv `
        --only-show-errors 2>$null

    if ($currentAuthMode -eq "aadOrApiKey" -and $currentFailureMode -eq "http403") {
        Write-Host "Search auth already configured (aadOrApiKey/http403)."
    } else {
