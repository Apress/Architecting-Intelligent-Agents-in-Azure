...
try:
    ...
    class AggregateContextProvider(ContextProvider):  # type: ignore[override]
        ...
        async def invoking(self, messages: Any, **kwargs: Any) -> Context:  # type: ignore[override]
            ...
            for provider in self._providers:
                ctx = await provider.invoking(messages, **kwargs)
                if getattr(ctx, "instructions", None):
                    instructions.append(ctx.instructions)
                if getattr(ctx, "messages", None):
                    collected_messages.extend(ctx.messages)
                if getattr(ctx, "tools", None):
                    collected_tools.extend(ctx.tools)

            merged_instructions = "\n\n".join(instructions) if instructions else None
            return Context(
                instructions=merged_instructions,
                messages=collected_messages or None,
...
try:
    ...
    AgentRunResponse = Any  # type: ignore
...
from services.reliability import (
    ...
    reset_reliability_event_callback,
    set_reliability_event_callback,
...
async def _handle_approval_status_request(
    ...
    try:
        ...
        _record_event(
            ...
            {
                "dependency": "cosmos",
                "fallback_path": "approval_status_unavailable",
                "degraded": True,
        ...
        return (
            f"Approval status for {approval_id} is temporarily unavailable due to a backend dependency issue. "
...
def _record_reliability_event(
    ...
    _record_event(recorder, event_type, payload, redactor=redactor)
...
def _build_degraded_triage_card(customer_message: str, board: Any, dependency: str) -> str:
    card = _build_triage_card(customer_message, board)
    marker = "**Suggest**"
    if marker not in card:
        return card

    before, _, after = card.partition(marker)
    degraded_suggest = (
        "Service is temporarily degraded while processing this request. "
        "Please retry in a few moments."
    )
    if dependency:
        degraded_suggest = (
            f"Service is temporarily degraded ({dependency} dependency). "
            "Please retry in a few moments."
        )
...
async def run_thain_agent(
    ...
    try:
        reliability_token = set_reliability_event_callback(
            lambda event_type, payload: _record_reliability_event(
                recorder,
                event_type,
                payload,
                redactor=trace_redactor,
        ...
        if approval_id:
            ...
            response = SimpleNamespace(
                text=status_text,
                messages=[ChatMessage(role="assistant", text=status_text)],
                metadata={},
            )
        ...
        if cache_hit and cached_result:
            ...
            response = SimpleNamespace(
                text=cached_text,
                messages=[ChatMessage(role="assistant", text=cached_text)],
                metadata={},
            )
            _record_event(
                recorder,
        ...
        async with AsyncExitStack() as stack:
            ...
            if response_mode != "normal":
                ...
                response = SimpleNamespace(
                    text=safety_text,
                    messages=[ChatMessage(role="assistant", text=safety_text)],
                    metadata={},
                )
            else:
                ...
                try:
                    ...
                    degraded_text = _build_degraded_triage_card(
                        customer_message,
                        board,
                        getattr(dep_exc, "dependency", "openai"),
                    )
                    _record_event(
                        recorder,
                        "fallback.used",
                        {
                            "dependency": getattr(dep_exc, "dependency", "openai"),
                            "fallback_path": "degraded_response",
                            "degraded": True,
                            "reason": type(dep_exc).__name__,
                        },
                        redactor=trace_redactor,
                    )
                    response = SimpleNamespace(
                        text=degraded_text,
                        messages=[ChatMessage(role="assistant", text=degraded_text)],
                        metadata={
                            "degraded": True,
                            "dependency": getattr(dep_exc, "dependency", "openai"),
                        },
        ...
        if isinstance(response_metadata, dict):
            degraded_value = response_metadata.get("degraded")
            if isinstance(degraded_value, bool):
                degraded_response = degraded_value
            elif degraded_value is not None:
                degraded_response = str(degraded_value).strip().lower() in {
                    "1",
                    "true",
                    "yes",
                    "y",
        ...
        reset_reliability_event_callback(reliability_token)
