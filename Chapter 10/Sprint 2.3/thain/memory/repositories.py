...
class CosmosRepository:
    ...
    def __init__(
        ...
        if not self._config.key and not credential:
            raise PersistentStoreError("Cosmos credential is required when COSMOS_KEY is not provided.")
    ...
    async def upsert_complaint(self, record: ComplaintRecordModel) -> None:
        ...
        try:
            ...
            await execute_dependency_call(
                "cosmos",
                "cosmos.items.upsert",
                lambda: container.upsert_item(payload),
            )
        except Exception as exc:
    ...
    async def fetch_recent(self, query: ComplaintQuery) -> list[ComplaintRecordModel]:
        ...
        try:
            async def _query_recent() -> list[ComplaintRecordModel]:
