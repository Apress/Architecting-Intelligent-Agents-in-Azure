param(
    ...
    [string]$EvalOpenAISecretName = "thain-openai-api-key",
...
$secretMap = @{
    ...
    "AZURE_OPENAI_EVAL_API_KEY" = $EvalOpenAISecretName
}
...
if ($ScrubEnv -and -not $seedFailures) {
    ...
    if ($EvalOpenAISecretName) {
        $overrideLines += "KV_EVAL_OPENAI_API_KEY_NAME=$EvalOpenAISecretName"
    }
...
if ($ConfigureContainerApp) {
    ...
    if ($EvalOpenAISecretName) {
        $overrides += "KV_EVAL_OPENAI_API_KEY_NAME=$EvalOpenAISecretName"
    }
