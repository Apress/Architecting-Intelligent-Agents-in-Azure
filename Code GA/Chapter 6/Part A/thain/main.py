...
semantic_service: SemanticRecallService | None = (
    SemanticRecallService(semantic_search_config) if semantic_search_config else None
)
...
def _wrap_tool(tool: Any, recorder: TraceRecorder) -> Any:
    ...
    if isinstance(tool, FunctionTool) and getattr(tool, "func", None) is not None:
        ...
        async def traced(*args: Any, **kwargs: Any) -> Any:
            ...
            recorder.record("tool.call", {"tool_name": name, "args": redacted_args})
            ...
            if (
                ...
                recorder.record(
                    "tool.result",
                    {
                        ...
                        "status": "error",
                        "duration_ms": duration_ms,
                        "error_type": error_info.get("error_type", "ToolError"),
                    },
                )
                return []
            recorder.record(
...
async def run_thain_agent(
    ...
    recorder.record("request.received", {"message_len": len(customer_message.strip()), "has_urls": has_urls})
    ...
    tools_list = [_wrap_tool(tool, recorder) for tool in tools_list]
    ...
    recorder.record("response.ready", {"category": normalized["category"], "summary": "<redacted>", "summary_len": len(str(normalized["summary"])), "elapsed_ms": elapsed_ms})
    trace_output_dir = os.getenv("TRACE_OUTPUT_DIR", "traces")
    sink = FileTraceSink(trace_output_dir)
    trace_path = sink.emit(recorder.to_dict())
    recorder.record("trace.emitted", {"path": trace_path})
