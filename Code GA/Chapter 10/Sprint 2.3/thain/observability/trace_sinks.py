...
class AppInsightsTraceSink:
    ...
    def emit(self, trace: dict[str, Any]) -> str:
        ...
        with self._tracer.start_as_current_span("thain.trace") as span:
            ...
            dependency_suppressed_set: set[str] = set()
            ...
            for event in events:
                ...
                if event_type == "llm.usage" and isinstance(data, dict):
                    llm_usage = data
                if event_type == "dependency.retry":
                    dependency_retry_count += 1
                if event_type == "dependency.failure":
                    dependency_failure_count += 1
                    if isinstance(data, dict):
                        dependency = data.get("dependency")
                        if dependency:
                            dependency_failures.add(str(dependency))
                if event_type == "dependency.suppressed":
                    dependency_suppressed = True
                    if isinstance(data, dict):
                        dependency = data.get("dependency")
                        if dependency:
                            dependency_suppressed_set.add(str(dependency))
                if event_type == "fallback.used" and isinstance(data, dict):
                    degraded_value = data.get("degraded")
                    if isinstance(degraded_value, bool):
                        degraded_fallback_used = degraded_fallback_used or degraded_value
                    elif degraded_value is not None:
                        degraded_fallback_used = degraded_fallback_used or (
                            str(degraded_value).strip().lower() in {"1", "true", "yes", "y", "on"}
                        )
                    path = data.get("fallback_path")
                    if path:
            ...
            span.set_attribute("thain.dependency.retry_count", dependency_retry_count)
            span.set_attribute("thain.dependency.failure_count", dependency_failure_count)
            span.set_attribute(
                "thain.dependency.suppressed",
                "true" if dependency_suppressed else "false",
            )
            span.set_attribute(
                "thain.fallback.used",
                "true" if degraded_fallback_used else "false",
            )
            span.set_attribute(
                "thain.reliability.degraded",
                "true" if degraded_fallback_used else "false",
            )
            span.set_attribute("thain.error.count", error_count)
            if dependency_failures:
                span.set_attribute("thain.dependency.failed", ",".join(sorted(dependency_failures)))
            if dependency_suppressed_set:
                span.set_attribute(
                    "thain.dependency.suppressed_names",
                    ",".join(sorted(dependency_suppressed_set)),
                )
            if fallback_paths:
                span.set_attribute("thain.fallback.path", ",".join(sorted(fallback_paths)))
            for event in events:
