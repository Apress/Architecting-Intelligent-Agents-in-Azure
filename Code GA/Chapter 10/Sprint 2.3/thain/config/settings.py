...
def validate_cloud_config() -> None:
    ...
    if _raw_env_flag("ENABLE_FEEDBACK", False):
        feedback_required = {
            "COSMOS_ENDPOINT": os.getenv("COSMOS_ENDPOINT", "").strip(),
            "COSMOS_DATABASE": os.getenv("COSMOS_DATABASE", "").strip(),
            "FEEDBACK_CONTAINER": os.getenv("FEEDBACK_CONTAINER", "").strip(),
        }
        missing_feedback = [name for name, value in feedback_required.items() if not value]
        if missing_feedback:
            raise MissingConfigError(
                "Missing required feedback configuration values: "
                f"{', '.join(sorted(missing_feedback))}."
            )
...
def load_feedback_enabled() -> bool:
    """Helper to load the feedback feature toggle from the environment."""

    return _env_flag("ENABLE_FEEDBACK", False)
...
class FeedbackConfig:
    """Configuration for Cosmos DB-backed feedback records."""
    ...
    @classmethod
    def from_env(cls) -> "FeedbackConfig | None":
        endpoint = os.getenv("COSMOS_ENDPOINT", "").strip()
        database = os.getenv("COSMOS_DATABASE", "").strip()
        container = os.getenv("FEEDBACK_CONTAINER", "").strip()
        ...
        key = os.getenv("COSMOS_KEY", "").strip() or None
        key = _strip_key_if_managed_identity(key)
        ttl_days_raw = os.getenv("FEEDBACK_TTL_DAYS", "").strip()
...
def load_feedback_config() -> FeedbackConfig | None:
    """Helper to load feedback configuration if available."""

    return FeedbackConfig.from_env()
