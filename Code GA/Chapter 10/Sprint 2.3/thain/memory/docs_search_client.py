...
class AzureDocsSearchClient:
    ...
    async def ensure_index(self, vector_dimensions: int) -> None:
        ...
        try:
            await execute_dependency_call(
                "search",
                "docs.index.get",
                lambda: self._index_client.get_index(self._index_name),
                passthrough_exceptions=(ResourceNotFoundError,),
            )
            ...
            logger.warning("Failed to check docs search index '%s': %s", self._index_name, exc, exc_info=True)
            raise DocsSearchError("Unable to access docs index metadata.") from exc
    ...
    async def upsert_document(self, document: dict[str, Any], embedding: list[float]) -> None:
        ...
        try:
            await execute_dependency_call(
                ...
                lambda: self._search_client.upload_documents(documents=[payload]),
            )
        except Exception as exc:
    ...
    async def search_documents(
        ...
        *,
        ...
        try:
            async def _query_docs() -> list[dict[str, Any]]:
