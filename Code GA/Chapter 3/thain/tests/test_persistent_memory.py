...
class PersistentContextProviderTests(unittest.IsolatedAsyncioTestCase):
    ...
    async def test_provider_handles_store_error(self) -> None:
        ...
        context.metadata = {"customer_id": "thain-demo"}
        await provider.before_run(agent=None, session=None, context=context, state={})
        # On store error, no instructions should be added
    ...
    async def test_provider_injects_instructions_when_records_present(self) -> None:
        service = MagicMock(spec=PersistentMemoryService)
        service.fetch_recent = AsyncMock(return_value=[_sample_record("WiFi issues")])
        ...
        self.assertIn("persistent memory", context.instructions[0])
        self.assertIn("Connectivity", context.instructions[0])
    ...
    async def test_provider_uses_metadata_customer_id(self) -> None:
        ...
        context = _make_mock_context()
        context.metadata = {"customer_id": "specific-customer"}
        await provider.before_run(agent=None, session=None, context=context, state={})
        service.fetch_recent.assert_awaited_once_with(customer_id="specific-customer", limit=5)
