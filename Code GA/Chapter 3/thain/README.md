...
This folder contains the Microsoft Agent Framework 1.5.0 GA version of the Chapter 3 code.
...
DevUI is a learning aid that makes tool calls, traces, and agent reasoning more visible than the command line. It is not required to run or validate the examples. Use the CLI or `pytest tests/` as the primary validation path.
...
- Retrieval currently returns the most recent complaints for the configured customer; Azure AI Search semantic recall is introduced in Chapter 4.
- If Cosmos DB is unavailable, Thain degrades gracefully and proceeds with in-memory context only.
