...
BASE_INSTRUCTIONS = (
    ...
    "and mention any meaningful connections or contrasts in the Insight section. "
...
class ThainDevAgent:
    ...
    async def run(self, messages: Any, **kwargs: Any) -> AgentRunResponse:
        ...
        # Ensure the DevUI receives the entity id in metadata for newer AF builds.
        try:
            if not hasattr(agent_response, "metadata") or agent_response.metadata is None:
                agent_response.metadata = {}           
                agent_response.metadata["entityId"] = self.id
        except Exception:
            pass
        try:
            meta = getattr(agent_response, "metadata", None)
            if isinstance(meta, dict):
                meta.setdefault("entityId", self.id)
        except Exception:
            pass
