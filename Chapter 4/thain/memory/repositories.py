...
class CosmosRepository:
    ...
    async def _ensure_container(self) -> Any:
        ...
        try:
            ...
            if self._config.ttl_seconds:
                try:
                    props = await self._container.read()
                    if props.get("defaultTtl") != self._config.ttl_seconds:
                        props["defaultTtl"] = self._config.ttl_seconds
                        await self._container.replace_container_properties(props)
                except Exception:
                    logger.debug("Could not update Cosmos container TTL.", exc_info=True)
        except Exception as exc:
